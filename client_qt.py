...
class TelaAutenticacao(QWidget):
    ...
    def configurar_sub_aba_cadastro(self):
        ...
        self.txt_cad_token.setPlaceholderText("Token de Autenticação API")
...
class TelaPartidaQuiz(QWidget):
    def __init__(self, quiz_id, perguntas, alternativas, callback_fim, sala_codigo_ativo=None):
        ...
        self.sala_codigo_ativo = sala_codigo_ativo  
        self.indice_atual = 0
    ...
    def finalizar_jogo(self):
        ...
        if self.sala_codigo_ativo:
            salas = carregar_json_local("data/salas.json")
            sala = next((s for s in salas if s["codigo"] == self.sala_codigo_ativo), None)
            if sala:
                ranking = sorted(sala["players"], key=lambda x: x["pontuacao"], reverse=True)
                msg_podio = "🏆 QUADRO FINAL DE RESULTADOS DA COMPETIÇÃO:\n\n"
                for i, p in enumerate(ranking):
                    medalha = "🥇" if i == 0 else "🥈" if i == 1 else "🥉" if i == 2 else "🔹"
                    msg_podio += f"{medalha} {p['nome']} — {p['pontuacao']} Pontos\n"
                QMessageBox.information(self, "Pódio da Arena", msg_podio)

                sala["players"] = [p for p in sala["players"] if p["id"] != ID_USUARIO_LOGADO]
                salvar_json_local("data/salas.json", salas)
...
class TelaDashboardJogo(QWidget):
    def __init__(self, callback_logout, callback_iniciar_partida):
        ...
        self.sala_codigo_ativa_lobby = None
        self.init_ui()
    ...
    # 🛠️ TELA 1 ATUALIZADA: Centraliza a listagem com o botão de excluir direto da lista principal
    def configurar_pag_salas(self):
        layout = QVBoxLayout(self.pag_salas)
        layout.setContentsMargins(35, 35, 35, 35)
        layout.setSpacing(10)

        self.stack_interno_salas = QStackedWidget()
        layout.addWidget(self.stack_interno_salas)

        self.widget_menu_salas = QWidget()
        layout_menu = QVBoxLayout(self.widget_menu_salas)
        layout_menu.setContentsMargins(0, 0, 0, 0)

        lbl_tit = QLabel("Salas Multiplayer")
        lbl_tit.setObjectName("lblMainHeader")
        layout_menu.addWidget(lbl_tit)

        self.lista_salas = QListWidget()
        layout_menu.addWidget(self.lista_salas)

        # 🛠️ ADICIONADO: Botão de exclusão reposicionado na Tela 1 de gerenciamento de Lobbies
        self.btn_excluir_sala_direto = QPushButton("❌ Excluir Sala Selecionada")
        self.btn_excluir_sala_direto.setObjectName("btnDanger")
        self.btn_excluir_sala_direto.clicked.connect(self.deletar_sala_por_clique_lista)
        layout_menu.addWidget(self.btn_excluir_sala_direto)

        self.txt_codigo_sala = QLineEdit()
        self.txt_codigo_sala.setPlaceholderText("Código de Entrada da Sala")
        layout_menu.addWidget(self.txt_codigo_sala)
        ...
        btn_entrar = QPushButton("🚪 Conectar na Sala")
        ...
        layout_menu.addLayout(layout_btns)
        self.stack_interno_salas.addWidget(self.widget_menu_salas)

        # 🛠️ TELA 2 ATUALIZADA: Mantém apenas a listagem e o botão limpo de voltar para o menu
        self.widget_lobby_ativo = QWidget()
        layout_lobby = QVBoxLayout(self.widget_lobby_ativo)
        layout_lobby.setContentsMargins(0, 0, 0, 0)

        self.lbl_titulo_sala_ativa = QLabel("Sala de Reunião: CODE")
        self.lbl_titulo_sala_ativa.setObjectName("lblMainHeader")
        layout_lobby.addWidget(self.lbl_titulo_sala_ativa)

        layout_lobby.addWidget(QLabel("<b>👥 Líderes e Competidores Conectados na Sala:</b>"))
        self.lista_jogadores_sala = QListWidget()
        layout_lobby.addWidget(self.lista_jogadores_sala)

        self.btn_start_arena = QPushButton("🎮 Pode começar a partida!")
        self.btn_start_arena.setObjectName("btnPrimary")
        self.btn_start_arena.clicked.connect(self.disparar_gameplay_sala_confirmada)
        layout_lobby.addWidget(self.btn_start_arena)

        # 🛠️ ALTERADO: Botão interno simplificado focado puramente em retornar/desconectar para a Tela 1 (Lobby Geral)
        btn_abandonar = QPushButton("← Voltar para o Menu")
        btn_abandonar.setObjectName("btnSecondary")
        btn_abandonar.clicked.connect(self.abandonar_lobby_parcial)
        layout_lobby.addWidget(btn_abandonar)

        self.stack_interno_salas.addWidget(self.widget_lobby_ativo)
        self.stack_interno_salas.setCurrentWidget(self.widget_menu_salas)

    def atualizar_salas(self):
        self.lista_salas.clear()
        for sala in carregar_json_local("data/salas.json"):
            qtd = len(sala.get("players", []))
            host_info = sala.get("host_id", "Desconhecido")[:8]
            self.lista_salas.addItem(f"🎮 Sala {sala['codigo']} │ Host: {host_info}... │ Competidores: {qtd}")
    ...
    def criar_sala(self):
        ...
        quiz_id = item.text().split(" - ID: ")[1].strip()
        salas = carregar_json_local("data/salas.json")
        codigo = gerar_codigo_sala()[:5] 

        usuarios = carregar_json_local("data/users.json")
        user_logado = next((u for u in usuarios if u["id"] == ID_USUARIO_LOGADO), None)
        nome_real = user_logado["nome"] if user_logado else "Líder Host"

        nova_sala = {
            "id": str(uuid.uuid4()), "codigo": codigo, "quiz_id": quiz_id,
            "host_id": ID_USUARIO_LOGADO, "status": "aguardando",
            "players": [{"id": ID_USUARIO_LOGADO, "nome": nome_real, "pontuacao": 0}]
        }
    ...
    def entrar_sala(self):
        ...
        self.sala_codigo_ativa_lobby = codigo
        usuarios = carregar_json_local("data/users.json")
        user_logado = next((u for u in usuarios if u["id"] == ID_USUARIO_LOGADO), None)
        nome_real = user_logado["nome"] if user_logado else f"Competidor_{str(uuid.uuid4())[:4]}"

        ja_existe = next((p for p in sala["players"] if p["id"] == ID_USUARIO_LOGADO), None)
        if not ja_existe:
            sala["players"].append({"id": ID_USUARIO_LOGADO, "nome": nome_real, "pontuacao": 0})
            salvar_json_local("data/salas.json", salas)
        ...
        for pos, player in enumerate(ranking):
            ...
            self.lista_jogadores_sala.addItem(f"{medalha} {player['nome']} [ONLINE] │ Placar: {player['pontuacao']} pts")

        self.lbl_titulo_sala_ativa.setText(f"Sala de Reunião: {codigo}")
        self.stack_interno_salas.setCurrentWidget(self.widget_lobby_ativo)

    def deletar_sala_por_clique_lista(self):
        item = self.lista_salas.currentItem()
        if not item:
            QMessageBox.warning(self, "Aviso", "Selecione uma sala na lista acima por clique para conseguir deletá-la!")
            return
            
        codigo_alvo = item.text().split("Sala ")[1].split(" │")[0].strip()
        salas = carregar_json_local("data/salas.json")
        sala = next((s for s in salas if s["codigo"] == codigo_alvo), None)
        
        if not sala: return
        
        if sala.get("host_id") == ID_USUARIO_LOGADO:
            salas = [s for s in salas if s["codigo"] != codigo_alvo]
            salvar_json_local("data/salas.json", salas)
            QMessageBox.information(self, "Sucesso", f"A Sala {codigo_alvo} foi completamente removida da base JSON!")
            self.atualizar_salas()
        else:
            QMessageBox.critical(self, "Erro de Permissão", "Você não é o Host criador desta sala para poder excluí-la!")

    def abandonar_lobby_parcial(self):
        if not self.sala_codigo_ativa_lobby: return
        salas = carregar_json_local("data/salas.json")
        sala = next((s for s in salas if s["codigo"] == self.sala_codigo_ativa_lobby), None)
        if sala:
            sala["players"] = [p for p in sala["players"] if p["id"] != ID_USUARIO_LOGADO]
            salvar_json_local("data/salas.json", salas)
        self.sala_codigo_ativa_lobby = None
    ...
    def disparar_gameplay_sala_confirmada(self):
        if not self.sala_codigo_ativa_lobby: return
        ...
        if sala:
            ...
            if perguntas:
                self.callback_iniciar_partida(quiz_id, perguntas, carregar_json_local("data/alternativas.json"), self.sala_codigo_ativa_lobby)
            else:
                QMessageBox.warning(self, "Aviso", "Este quiz selecionado pela sala não possui perguntas vinculadas.")
